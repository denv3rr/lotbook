...
export function BackupDatabase() {
  ...
  async function download(event: FormEvent) {
    ...
    try {
      ...
      const response = await fetch(`${getApiBase()}/api/application/backup`, { method: "POST", headers: { "Content-Type": "application/json", "X-API-Key": key, "X-Clear-Backup": "confirm" }, body: JSON.stringify({ confirm: true, passphrase }) });
      if (!response.ok) {
      ...
      anchor.href = url; anchor.download = `clear-database-${new Date().toISOString().slice(0, 10)}.clearbackup`;
  ...
  return <section className="mt-4 rounded-xl border border-slate-700 p-4 text-sm text-slate-200" aria-label="Database recovery">
    ...
    <Modal open={open} onClose={close} title="Export encrypted database backup" description="This exports all client records. Clear cannot recover a lost backup passphrase. Nothing in the running database is changed.">
