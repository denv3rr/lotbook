...
export function CloseApp({ onCancel, onStopping }: { onCancel: () => void; onStopping: () => void }) {
  ...
  return <Modal open title="Close Clear?" description="This closes the local API and dashboard server for this Clear session. Saved records stay on disk. Finish or save any open edits first." onClose={() => { if (!busy) onCancel(); }}>
