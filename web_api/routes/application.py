...
def application_status(request: Request):
    ...
    return attach_meta({"shutdown_available": available, "message": "Ready to close this Clear stack." if available else "Restart using the Clear launcher without --reload to enable safe app shutdown."}, route="/api/application/status", source="launcher")
...
def shutdown(payload: ShutdownRequest, request: Request, background: BackgroundTasks):
    ...
    if request.headers.get("x-clear-shutdown") != "confirm":
    ...
    if control is None or not control[1]["processes"].get("web") or not callable(callback):
        raise HTTPException(409, "This API was not started as a managed Clear stack. Restart using the Clear launcher without --reload.")
    ...
    if origin and origin not in (control[1]["ui_origin"], f"http://localhost:{expected.port}"):
        raise HTTPException(403, "Shutdown must come from this Clear dashboard.")
    background.add_task(callback)
    return attach_meta({"status": "stopping", "message": "Shutdown requested. Clear will finish active API work, then stop its UI process tree. You can close this browser tab."}, route="/api/application/shutdown", source="launcher")
